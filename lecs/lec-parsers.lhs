...
bindP (P pA) pB = P (\cs -> [(xb, cs'') | (xa, cs')  <- doParse pA cs,
                                         (xb , cs'') <- (pB xa) cs')])
-}
...
charP  c = satP (c ==)
strP :: String -> parser TSring
strP [] = return ""
-- above doesnt consume anything, simply return s empty string
