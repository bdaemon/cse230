...
>   pure    = error "DO IN CLASS 1"
>   _ <*> _ = error "DO IN CLASS 2"
...
> instance Applicative m => Applicative (ExcT m) where
...
> evalExSt :: Expr -> ExSt Int
> evalExSt = evalMega
>
> evalStEx :: Expr -> StEx Int
> evalStEx = evalMega

we can *extract* the values from the monadic action by *running* the actions
from a given *start* state (here the initial counter `0`)

> runStEx :: StEx a -> Exc (a, Int)
> runStEx (MkSTT f) = f 0
>
> runExSt :: ExSt a -> (Exc a, Int)
> runExSt (MkExc (S f)) = f 0

which gives the rendering functions:
...
we can now *evaluate* the expressions in GHCi:
...
we can create a concrete instance by defining:

> type SE a = StateT Int (Either String) a
...
Which lets us evaluate an expression as:
...
**Note:** the different types of `evalES`, `evalSE` and `runES`, `runSE`.
These arise from the different order in which the monads are *stacked*.
...
> runWSE m = runStateT (runWriterT m) 0

~~~~~{.haskell}
ghci> runWSE (evalWSE ok)
