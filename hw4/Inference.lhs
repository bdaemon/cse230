...
mgu (l `TCom` r) (l' `TCom` r')  = do  s1 <- mgu l l'
                                       ...
                                       return (s2 `after` s1)
mgu (TList t1) (TList t2)        = do  s <- mgu t1 t2
                                       return s
...
inferListType :: (MonadState TIState m, MonadError String m) => TypeEnv -> Exp -> m (Subst, Type)
inferListType env e =
    do
        tv        <- freshTVbl "a"
        (s1, t1)  <- ti env e
...
ti_pair env e          =
    do
        ...
        s2      <- mgu t (t1 `TCom` t2)
        let s3   = s2 `after` s1
        return (s3, apply s3 t1, apply s3 t2)
...
ti env (e1 `ECons` e2) =
    do
        (s1, t1) <- ti env e1
        (s2, t2) <- inferListType (apply s1 env) e2
        s3       <- mgu (apply s2 t1) t2
        return (s3 `after` s2 `after` s1, apply s3 (TList t2))

ti env (EIsNil e)      =
    do
        (s1, t)   <- inferListType env e
        return (s1, TBool)

ti env (EDcons e)      =
    do
        (s1, t1) <- inferListType env e
...
prExp (EAbs x e)       =   PP.char '\\' PP.<+> prEVbl x PP.<+>
                           PP.text "->" PP.<+>
                           prExp e
prExp (e1 `ECom` e2)   =   PP.text "(" PP.<+> prExp e1 PP.<+>
...
prScheme (Forall as t)  =   PP.text "For All" PP.<+>
